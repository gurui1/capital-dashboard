  ...
  <div class="wBox">
    ...
    <div class="bodyBottom">
      ...
      <el-table-column align="center" prop="日期" label="日期" width="200" />
        ...
        <el-table-column align="center" prop="实际支出金额" label="实际支出金额(人民币)" />
      </el-table>
      <div class="last">
...
import { biaoFive,  } from "@/api/dataBoard/api";
import { exportExcel } from "../../excelConfig"
import { excelFive } from "@/api/dataBoard/excel"; 
...
let toExcel = () => {
  ...
  .then((res) => {
      ...
      link.href = url;
      // // 设置下载的文件名，包括文件扩展名
      link.setAttribute('download', '实际支出表.xlsx');
...
let getData =async () => {
  ...
  try {
    const response = await biaoFive(payLoad);
