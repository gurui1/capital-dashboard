  ...
  <div class="wBox">
      ...
      <div class="search">
        <div class="top">
          ...
          <div class="three">
            ...
            <el-option v-for="item in selectDataStore.belongBu" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
          </div>
          <div class="four">
            <span>&nbsp;&nbsp;实际回款金额: &nbsp;&nbsp;&nbsp;</span>
        ...
        <div class="bottom">
          <div class="three">
            ...
            <el-option v-for="item in selectDataStore.firstBusiness" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
          </div>
          <div class="three">
            <span>&nbsp;&nbsp;二级业务: &nbsp;&nbsp;&nbsp;</span>
            ...
            <el-option v-for="item in selectDataStore.secondBusiness" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
          </div>
          <div class="three">
            <span>&nbsp;&nbsp;部门: &nbsp;&nbsp;&nbsp;</span>
    ...
    <div class="bodyBottom">
      ...
      <el-table-column align="center" prop="日期" label="日期" width="200" />
        <el-table-column align="center" prop="账户名称" label="账户名称" width="250" />
        ...
        <el-table-column align="center" prop="实际回款金额" label="实际回款金额(人民币)" />
      </el-table>
      <div class="last">
        <div class="lastone" style="position:relative;left: 0px;width: 200px;">总计</div>
        <div class="lastone" style="position:relative;left: 1020px;">{{ renminbi }}</div>
      </div>
...
import { biaoSix,  } from "@/api/dataBoard/api";
import { exportExcel } from "../../excelConfig"
import { excelSix } from "@/api/dataBoard/excel"; 
...
let toExcel = () => {
  let payLoad = reactive({
    ...
    startReturnMoney:yuJi1.value,
    endReturnMoney:yuJi2.value,
    firstBusiness: yijiYW.value,
    secondBusiness: erjiYW.value,
    oragename:buMen.value,
  ...
  .then((res) => {
      ...
      link.href = url;
      // // 设置下载的文件名，包括文件扩展名
      link.setAttribute('download', '预计回款表.xlsx');
...
let getData =async () => {
  ...
  try {
    const response = await biaoSix(payLoad);
    const resArr = response.data.data.pageInfo.list;
    tableData.splice(0, tableData.length); // 清空原有数据
    resArr.forEach((item: { incomeDate: any; company: any; belongBu: any; firstBusiness: any; secondBusiness: any; oragename: any; recoverMoney: any; }) => {
      tableData.push({
        日期: item.incomeDate,
        账户名称: item.company,
        账号简称: item.shortName,
        ...
        部门: item.oragename,
        实际回款金额: item.recoverMoney
      });
    })
    total.value = response.data.data.pageInfo.total;
    renminbi.value = response.data.data.recoverMoneyTotal;
  } catch (error) {
