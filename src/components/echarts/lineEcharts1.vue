<template>
    <div id="all1">
        ...
        <div class="select">
            ...
            <span v-if="props.panduan == false">&nbsp;{{ timeLineVal[0] }}&nbsp;&nbsp;至&nbsp;&nbsp;{{ timeLineVal[1]
                }}</span>

            <el-radio-group v-model="radio">
                <el-radio :value="0">归属体系</el-radio>
                <el-radio :value="1">归属部门</el-radio>
...
import { biao3, biao4, biao5, biao6 } from "@/api/dataBoard/data";
...
let getData = () => {
    ...
    else if (props.title == '预计回款金额部门统计') {
        biao4(payLoad2).then((res) => {
            // console.log(res, '预计回款金额部门统计');
            let resArr = res.data.data;
            echartX.value = []
            echarty.value = []
            Object.entries(resArr).forEach(([key, value]) => {
    ...
    } else if (props.title == '实际支出金额部门统计') {
        biao5(payLoad).then((res) => {
            let resArr = res.data.data;
            echartX.value = []
            echarty.value = []
            Object.entries(resArr).forEach(([key, value]) => {
    ...
    } else if (props.title == '实际回款金额部门统计') {
        biao6(payLoad3).then((res) => {
            let resArr = res.data.data;
            echartX.value = []
            echarty.value = []
            Object.entries(resArr).forEach(([key, value]) => {
